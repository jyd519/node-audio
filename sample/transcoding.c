...
int pad_last_frame(AVCodecContext *s, AVFrame *frame, const AVFrame *src);
...
int prepare_video_encoder(StreamingContext *sc, AVCodecContext *decoder_ctx, AVRational input_framerate, StreamingParams sp) {
  ...
  sc->frame_buffer = av_frame_alloc();
...
int encode_audio(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame) {
  ...
  if (input_frame->nb_samples < encoder->audio_avcc->frame_size) {
    pad_last_frame(encoder->audio_avcc, encoder->frame_buffer, input_frame);
  } else {
    av_frame_ref(encoder->frame_buffer, input_frame);
  }

  int response = avcodec_send_frame(encoder->audio_avcc, encoder->frame_buffer);

  av_frame_unref(encoder->frame_buffer);
...
int main(int argc, char *argv[])
{
  ...
  sp.copy_audio = 0;
  sp.copy_video = 0;
  sp.video_codec = "libopenh264"; //"libx264";
  sp.audio_codec = "aac";
  sp.codec_priv_key = "x264-params";
  sp.codec_priv_value = "keyint=60:min-keyint=60:scenecut=0:force-cfr=1";
...
int pad_last_frame(AVCodecContext *s, AVFrame *frame, const AVFrame *src) {
  ...
  if ((ret = av_samples_copy(frame->extended_data, src->extended_data, 0, 0, src->nb_samples,
                             s->channels, s->sample_fmt)) < 0)
    goto fail;
  if ((ret = av_samples_set_silence(frame->extended_data, src->nb_samples,
                                    frame->nb_samples - src->nb_samples, s->channels,
                                    s->sample_fmt)) < 0)
    goto fail;
