...
int main(int argc, char **argv) {
  ...
  Codec ocodec = findEncodingCodec(AV_CODEC_ID_H264);
  ...
  encoder.setBitRate(3000*1000);
  // encoder.setBitRate(vdec.bitRate());
  encoder.addFlags(octx.outputFormat().isFlags(AVFMT_GLOBALHEADER) ? AV_CODEC_FLAG_GLOBAL_HEADER
                                                                   : 0);
  ...
  while (true) {
    ...
    if (pkt) {
      ...
      clog << "Read packet: pts=" << pkt.pts() << ", dts=" << pkt.dts() << " / "
           << pkt.pts().seconds() << " / " << pkt.timeBase() << " / st: " << pkt.streamIndex()
           << endl;
    } else {
      flushDecoder = true;
    }
    ...
    do {
      ...
      if (frame) {
        clog << "Frame: pts=" << frame.pts() << " / " << frame.pts().seconds() << " / "
             << frame.timeBase() << ", " << frame.width() << "x" << frame.height()
             << ", size=" << frame.size() << ", ref=" << frame.isReferenced() << ":"
             << frame.refCount() << " / type: " << frame.pictureType() << endl;
