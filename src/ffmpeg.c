...
static void print_report(int is_last_report, int64_t timer_start,
  ...
  if (pts == AV_NOPTS_VALUE) {
    ...
    av_bprintf(&buf, "%s%02" PRId64 ":%02d:%02d.%02d ", hours_sign, hours, mins,
...
static void print_stream_maps(void) {
    ...
    for (int j = 0; j < ist->nb_filters; j++) {
      if (!filtergraph_is_simple(ist->filters[j]->graph)) {
        av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d (%s) -> %s", ist->file_index,
               ist->index, ist->dec ? ist->dec->name : "?",
               ist->filters[j]->name);
  ...
  for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
    ...
    av_log(NULL, AV_LOG_INFO, "  Stream #%d:%d -> #%d:%d", ost->ist->file_index,
           ost->ist->index, ost->file_index, ost->index);
    if (ost->enc_ctx) {
...
static int choose_output(OutputStream **post) {
  ...
  for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
    ...
    if (ost->filter && ost->filter->last_pts != AV_NOPTS_VALUE) {
      ...
      opts =
          ost->last_mux_dts == AV_NOPTS_VALUE ? INT64_MIN : ost->last_mux_dts;
    }
...
static int check_keyboard_interaction(int64_t cur_time) {
  ...
  if (key == 'c' || key == 'C') {
    ...
    if (k > 0 && (n = sscanf(buf, "%63[^ ] %lf %255[^ ] %255[^\n]", target,
                             &time, command, arg)) >= 3) {
      av_log(NULL, AV_LOG_DEBUG,
             "Processing command target:%s time:%f command:%s arg:%s", target,
             time, command, arg);
      for (i = 0; i < nb_filtergraphs; i++)
  ...
  for (ist = ist_iter(NULL); ist; ist = ist_iter(ist)) {
    ...
    err_rate =
        (ist->frames_decoded || ist->decode_errors)
            ? ist->decode_errors / (ist->frames_decoded + ist->decode_errors)
            : 0.f;
...
void log_callback_capture(void *ptr, int level, const char *fmt, va_list vl) {
  int available_space = 0;
  // Lock the mutex before writing to the log buffer
  pthread_mutex_lock(&log_mutex);

  // Calculate the available space in the buffer
  available_space = log_buffer_size - log_buffer_offset;

  // Check if the remaining space is not enough
  if (vsnprintf(NULL, 0, fmt, vl) >= available_space) {
    // Resize the buffer to accommodate the new log message
    int new_size = log_buffer_size + LOG_BUFFER_SIZE;
    log_buffer = (char *)av_realloc(log_buffer, new_size);
    if (!log_buffer) {
      fprintf(stderr, "Error: Failed to resize log_buffer\n");
  ...
  // Use vsnprintf to safely format the log message into the buffer
  int offset =
      vsnprintf(log_buffer + log_buffer_offset, available_space, fmt, vl);

  log_buffer_offset += offset;

  // Unlock the mutex after writing to the log buffer
  pthread_mutex_unlock(&log_mutex);
}
...
EXPORTED void ff_free(void *p) { av_free(p); }
EXPORTED void ff_stop_transcode() {
  received_sigterm = 1;
}
#endif

#ifdef BUILD_DLL
EXPORTED int ff_run(int argc, char **argv)
#else
