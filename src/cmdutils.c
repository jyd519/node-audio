...
int opt_default(void *optctx, const char *opt, const char *arg) {
  ...
  if (!consumed &&
    ...
    if (!strcmp(opt, "srcw") || !strcmp(opt, "srch") || !strcmp(opt, "dstw") ||
        !strcmp(opt, "dsth") || !strcmp(opt, "src_format") ||
        !strcmp(opt, "dst_format")) {
      av_log(NULL, AV_LOG_ERROR,
...
FILE *get_preset_file(char *filename, size_t filename_size,
  ...
  if (is_path) {
    ...
    for (i = 0; i < 3 && !f; i++) {
      ...
      f = fopen_utf8(filename, "r");
      if (!f && codec_name) {
        snprintf(filename, filename_size, "%s%s/%s-%s.ffpreset", base[i],
                 i != 1 ? "" : "/.ffmpeg", codec_name, preset_name);
        f = fopen_utf8(filename, "r");
      }
    }
  }
...
int setup_find_stream_info_opts(AVFormatContext *s, AVDictionary *codec_opts,
  ...
  *dst = NULL;
  ...
  for (int i = 0; i < s->nb_streams; i++) {
    ret = filter_codec_opts(codec_opts, s->streams[i]->codecpar->codec_id, s,
                            s->streams[i], NULL, &opts[i]);
    if (ret < 0)
      goto fail;
  }
  *dst = opts;
