...
struct ProbeContext {
  ...
  struct ReadInterval *read_intervals;
...
#define SECTION_FLAG_HAS_VARIABLE_FIELDS                                       \
  4 ///< the section may contain a variable number of fields with variable keys.
...
static const char *get_packet_side_data_type(void *data) {
  ...
  return (const char*)av_x_if_null(av_packet_side_data_name(sd->type), "unknown");
}

static const char *get_frame_side_data_type(void *data) {
  const AVFrameSideData *sd = (const AVFrameSideData *)data;
  return (const char*)av_x_if_null(av_frame_side_data_name(sd->type), "unknown");
}
...
#if HAVE_THREADS
// static pthread_once_t once_init_log = PTHREAD_ONCE_INIT;
...
static char *value_string(char *buf, int buf_size, struct unit_value uv, const struct ProbeContext* pctx) {
  double vald;
  long long int vali;
  int show_float = 0;
  int show_value_unit = pctx->show_value_unit;
  ...
  if (uv.unit == unit_second_str && pctx->use_value_sexagesimal_format) {
    double secs;
    int hours, mins;
    secs = vald;
    mins = (int)secs / 60;
    ...
    if (pctx->use_value_prefix && vald > 1) {
      long long int index;

      if (uv.unit == unit_byte_str && pctx->use_byte_value_binary_prefix) {
    ...
    if (show_float || (pctx->use_value_prefix && vald != (long long int)vald))
...
#ifdef BUILD_DLL
    avio_close_dyn_buf((*wctx)->avio, &p);
    av_free(p);
#else
...
static int writer_open(WriterContext **wctx, const Writer *writer,
                       ...
                       int nb_sections, const char *output, struct ProbeContext* pctx) {
  int i, ret = 0;
  ...
  if (args) {
    ...
    while ((opt = av_dict_iterate(opts, opt))) {
      if ((ret = av_opt_set(*wctx, opt->key, opt->value,
        ...
        av_log(*wctx, AV_LOG_ERROR,
               "Failed to set option '%s' with value '%s' provided to writer "
               "context\n",
               opt->key, opt->value);
        av_dict_free(&opts);
        goto fail;
  ...
  if (!pctx->output_filename) {
#ifdef BUILD_DLL
    if (avio_open_dyn_buf(&(*wctx)->avio) < 0) {
      av_log(*wctx, AV_LOG_ERROR, "Failed to open output '%s' with error: %s\n",
...
static inline int validate_string(WriterContext *wctx, char **dstp,
  ...
  for (p = (uint8_t *)src; *p;) {
    ...
    if (av_utf8_decode(&code, &p, endp, wctx->string_validation_utf8_flags) <
        0) {
      AVBPrint bp;
      av_bprint_init(&bp, 0, AV_BPRINT_SIZE_AUTOMATIC);
      bprint_bytes(&bp, p0, p - p0);
...
static inline int writer_print_string(WriterContext *wctx, const char *key,
  ...
  const struct section *section = wctx->section[wctx->level];
  int ret = 0;
  const struct ProbeContext* pctx = wctx->pctx;

  if (pctx->show_optional_fields == SHOW_OPTIONAL_FIELDS_NEVER ||
      (pctx->show_optional_fields == SHOW_OPTIONAL_FIELDS_AUTO &&
...
static void writer_print_time(WriterContext *wctx, const char *key, int64_t ts,
  ...
  if ((!is_duration && ts == AV_NOPTS_VALUE) || (is_duration && ts == 0)) {
    ...
    value_string(buf, sizeof(buf), uv, wctx->pctx);
...
static void writer_print_data_hash(WriterContext *wctx, const char *name,
  ...
  if (!pctx->hash)
    return;
  av_hash_init(pctx->hash);
  av_hash_update(pctx->hash, data, size);
  snprintf(buf, sizeof(buf), "%s:", av_hash_get_name(pctx->hash));
  p = buf + strlen(buf);
  av_hash_final_hex(pctx->hash, p, buf + sizeof(buf) - p);
...
static void compact_print_section_header(WriterContext *wctx, void *data) {
  ...
  if (parent_section &&
    ...
    av_bprintf(section_pbuf, "%s%s", wctx->section_pbuf[wctx->level - 1].str,
               element_name);
...
static void json_print_section_header(WriterContext *wctx, void *data) {
  ...
  if (section->flags & SECTION_FLAG_IS_WRAPPER) {
    ...
    if (section->flags & SECTION_FLAG_IS_ARRAY) {
      ...
      /* this is required so the parser can distinguish between packets and
       * frames */
      if (parent_section &&
...
#define CHECK_COMPLIANCE(opt, opt_name)                                        \
  if (opt) {                                                                   \
    ...
    CHECK_COMPLIANCE(pctx->show_private_data, "private");
    CHECK_COMPLIANCE(pctx->show_value_unit, "unit");
    CHECK_COMPLIANCE(pctx->use_value_prefix, "prefix");
  }
...
static void xml_print_section_header(WriterContext *wctx, void *data) {
  ...
  if (section->flags &
    ...
    if (section->flags & SECTION_FLAG_HAS_TYPE) {
      AVBPrint buf;
      av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
      av_bprint_escape(&buf, section->get_type(data), NULL, AV_ESCAPE_MODE_XML,
...
static void xml_print_value(WriterContext *wctx, const char *key,
  ...
  if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS) {
    ...
    if (is_int) {
      ...
      av_bprint_escape(&buf, (const char *)value, NULL, AV_ESCAPE_MODE_XML,
                       AV_ESCAPE_FLAG_XML_DOUBLE_QUOTES);
...
#define print_val(k, v, u)                                                     \
  do {                                                                         \
    ...
    uv.val.i = v;                                                              \
    uv.unit = u;                                                               \
    writer_print_string(w, k, value_string(val_str, sizeof(val_str), uv, pctx), 0);  \
  } while (0)
...
static void print_dovi_metadata(WriterContext *w, const AVDOVIMetadata *dovi) {
...

  {
    ...
    print_int("mapping_chroma_format_idc", mapping->mapping_chroma_format_idc);
    ...
    for (int c = 0; c < 3; c++) {
      ...
      for (int i = 0; i < curve->num_pivots - 1; i++) {
        ...
        case AV_DOVI_MAPPING_POLYNOMIAL:
          ...
          print_list_fmt("poly_coef", "%" PRIi64, curve->poly_order[i] + 1, 1,
        ...
        case AV_DOVI_MAPPING_MMR:
          ...
          print_list_fmt("mmr_coef", "%" PRIi64, curve->mmr_order[i], 7,
...
static void print_dynamic_hdr10_plus(WriterContext *w,
  ...
  for (int n = 1; n < metadata->num_windows; n++) {
    ...
    print_q("window_upper_left_corner_x", params->window_upper_left_corner_x,
            '/');
    print_q("window_upper_left_corner_y", params->window_upper_left_corner_y,
            '/');
    print_q("window_lower_right_corner_x", params->window_lower_right_corner_x,
            '/');
    print_q("window_lower_right_corner_y", params->window_lower_right_corner_y,
            '/');
    print_q("window_upper_left_corner_x", params->window_upper_left_corner_x,
            '/');
    print_q("window_upper_left_corner_y", params->window_upper_left_corner_y,
  ...
  for (int n = 0; n < metadata->num_windows; n++) {
    ...
    if (params->tone_mapping_flag) {
      ...
      print_int("num_bezier_curve_anchors", params->num_bezier_curve_anchors);
...
static void print_pkt_side_data(WriterContext *w, AVCodecParameters *par,
  ...
  const struct ProbeContext* pctx = w->pctx;
...
static void show_packet(WriterContext *w, InputFile *ifile, AVPacket *pkt,
  ...
  if (pkt->side_data_elems) {
    ...
    if (side_metadata && size && pctx->do_show_packet_tags) {
...
static void show_frame(WriterContext *w, AVFrame *frame, AVStream *stream,
  ...
  if (pctx->do_show_frame_tags)
    show_tags(w, frame->metadata, SECTION_ID_FRAME_TAGS);
  if (pctx->do_show_log)
    show_log(w, SECTION_ID_FRAME_LOGS, SECTION_ID_FRAME_LOG, pctx->do_show_log);
...
static av_always_inline int process_frame(WriterContext *w, InputFile *ifile,
  ...
  if (got_frame) {
    ...
    pctx->nb_streams_frames[pkt->stream_index]++;
    if (pctx->do_show_frames)
      if (is_sub)
...
static int read_interval_packets(WriterContext *w, InputFile *ifile,
  ...
  struct ProbeContext* pctx = w->pctx;
  ...
  while (!av_read_frame(fmt_ctx, pkt)) {
    if (fmt_ctx->nb_streams > pctx->nb_streams) {
      REALLOCZ_ARRAY_STREAM(pctx->nb_streams_frames, pctx->nb_streams, fmt_ctx->nb_streams);
      REALLOCZ_ARRAY_STREAM(pctx->nb_streams_packets, pctx->nb_streams,
                            fmt_ctx->nb_streams);
      REALLOCZ_ARRAY_STREAM(pctx->selected_streams, pctx->nb_streams, fmt_ctx->nb_streams);
      pctx->nb_streams = fmt_ctx->nb_streams;
    }
    if (pctx->selected_streams[pkt->stream_index]) {
      ...
      frame_count++;
      if (pctx->do_read_packets) {
        if (pctx->do_show_packets)
          show_packet(w, ifile, pkt, i++);
        pctx->nb_streams_packets[pkt->stream_index]++;
      }
      if (pctx->do_read_frames) {
        int packet_new = 1;
        FrameData *fd;
...
static int read_packets(WriterContext *w, InputFile *ifile) {
  ...
  if (pctx->read_intervals_nb == 0) {
    ReadInterval interval = (ReadInterval){.has_start = 0, .has_end = 0};
    ret = read_interval_packets(w, ifile, &interval, &cur_ts);
  } else {
    for (i = 0; i < pctx->read_intervals_nb; i++) {
      ret = read_interval_packets(w, ifile, &pctx->read_intervals[i], &cur_ts);
      if (ret < 0)
        break;
    }
  }
...
static int show_stream(WriterContext *w, AVFormatContext *fmt_ctx,
  ...
  case AVMEDIA_TYPE_VIDEO:
    ...
    if (dec_ctx) {
      ...
      print_int("closed_captions",
                !!(dec_ctx->properties & FF_CODEC_PROPERTY_CLOSED_CAPTIONS));
  ...
  if (pctx->nb_streams_frames[stream_idx])
    print_fmt("nb_read_frames", "%" PRIu64, pctx->nb_streams_frames[stream_idx]);
  else
    print_str_opt("nb_read_frames", "N/A");
  if (pctx->nb_streams_packets[stream_idx])
    print_fmt("nb_read_packets", "%" PRIu64, pctx->nb_streams_packets[stream_idx]);
  else
...
#define PRINT_DISPOSITION(flagname, name)                                      \
  ...
  if (pctx->do_show_stream_disposition) {
    writer_print_section_header(w, NULL,
                                in_program
                                    ? SECTION_ID_PROGRAM_STREAM_DISPOSITION
                                    : SECTION_ID_STREAM_DISPOSITION);
...
static int show_program(WriterContext *w, InputFile *ifile,
  ...
  for (i = 0; i < program->nb_stream_indexes; i++) {
    if (pctx->selected_streams[program->stream_index[i]]) {
...
static int open_input_file(InputFile *ifile, const char *filename,
                           const char *print_filename, const struct ProbeContext* pctx) {
  int err, i;
  ...
  if ((err = avformat_open_input(&fmt_ctx, filename, pctx->iformat, &format_opts)) <
      0) {
  ...
  for (i = 0; i < fmt_ctx->nb_streams; i++) {
    ...
    }
    {
      ...
      if (pctx->do_show_log) {
        ...
        // otherwise the log information would need to be reordered and matches
        // up to contexts and frames That is in fact possible but not trivial
...
static int probe_file(WriterContext *wctx, const char *filename,
  ...
  int ret, i;
  int section_id;
  struct ProbeContext* pctx = wctx->pctx;

  ((struct ProbeContext*)pctx)->do_read_frames = pctx->do_show_frames || pctx->do_count_frames;
  ((struct ProbeContext*)pctx)->do_read_packets = pctx->do_show_packets || pctx->do_count_packets;

  ret = open_input_file(&ifile, filename, print_filename, pctx);
  if (ret < 0)
    goto end;
...
#define CHECK_END                                                              \
  ...
  pctx->nb_streams = ifile.fmt_ctx->nb_streams;
  REALLOCZ_ARRAY_STREAM(pctx->nb_streams_frames, 0, ifile.fmt_ctx->nb_streams);
  REALLOCZ_ARRAY_STREAM(pctx->nb_streams_packets, 0, ifile.fmt_ctx->nb_streams);
  REALLOCZ_ARRAY_STREAM(pctx->selected_streams, 0, ifile.fmt_ctx->nb_streams);

  for (i = 0; i < ifile.fmt_ctx->nb_streams; i++) {
    if (pctx->stream_specifier) {
      ret = avformat_match_stream_specifier(
          ifile.fmt_ctx, ifile.fmt_ctx->streams[i], pctx->stream_specifier);
      CHECK_END;
      else pctx->selected_streams[i] = ret;
      ret = 0;
    } else {
      pctx->selected_streams[i] = 1;
    }
    if (!pctx->selected_streams[i])
      ifile.fmt_ctx->streams[i]->discard = AVDISCARD_ALL;
  }

  if (pctx->do_read_frames || pctx->do_read_packets) {
    if (pctx->do_show_frames && pctx->do_show_packets &&
        wctx->writer->flags &
            WRITER_FLAG_PUT_PACKETS_AND_FRAMES_IN_SAME_CHAPTER)
      section_id = SECTION_ID_PACKETS_AND_FRAMES;
    else if (pctx->do_show_packets && !pctx->do_show_frames)
  ...
  if (pctx->do_show_programs) {
    ret = show_programs(wctx, &ifile);
    CHECK_END;
  }

  if (pctx->do_show_streams) {
    ret = show_streams(wctx, &ifile);
    CHECK_END;
  }
  if (pctx->do_show_chapters) {
    ret = show_chapters(wctx, &ifile);
    CHECK_END;
  }
  if (pctx->do_show_format) {
  ...
  av_freep(&pctx->nb_streams_frames);
  av_freep(&pctx->nb_streams_packets);
  av_freep(&pctx->selected_streams);
...
static void ffprobe_show_pixel_formats(WriterContext *w) {
  ...
  while (pixdesc = av_pix_fmt_desc_next(pixdesc)) {
    ...
    if (pctx->do_show_pixel_format_flags) {
    ...
    if (pctx->do_show_pixel_format_components && (pixdesc->nb_components > 0)) {
...
static int opt_show_optional_fields(void *optctx, const char *opt,
                                    const char *arg) {
  struct ProbeContext* pctx = optctx; 
  if (!av_strcasecmp(arg, "always"))
    pctx->show_optional_fields = SHOW_OPTIONAL_FIELDS_ALWAYS;
  else if (!av_strcasecmp(arg, "never"))
    pctx->show_optional_fields = SHOW_OPTIONAL_FIELDS_NEVER;
  else if (!av_strcasecmp(arg, "auto"))
    pctx->show_optional_fields = SHOW_OPTIONAL_FIELDS_AUTO;

  if (pctx->show_optional_fields == SHOW_OPTIONAL_FIELDS_AUTO &&
...
static int opt_format(void *optctx, const char *opt, const char *arg) {
  struct ProbeContext* pctx = optctx; 
  pctx->iformat = av_find_input_format(arg);
  if (!pctx->iformat) {
...
static int opt_input_file(void *optctx, const char *arg) {
  struct ProbeContext* pctx = optctx; 
  if (pctx->input_filename) {
    av_log(NULL, AV_LOG_ERROR,
           "Argument '%s' provided as input filename, but '%s' was already "
           "specified.\n",
           arg, pctx->input_filename);
    return AVERROR(EINVAL);
  }
  if (!strcmp(arg, "-"))
    arg = "fd:";
  pctx->input_filename = arg;
...
static int opt_output_file_o(void *optctx, const char *opt, const char *arg) {
  ...
  if (pctx->output_filename) {
    av_log(NULL, AV_LOG_ERROR,
           ...
           "specified.\n",
           arg, pctx->output_filename);
    return AVERROR(EINVAL);
  }
  if (!strcmp(arg, "-"))
    arg = "fd:";
  pctx->output_filename = arg;
...
static int opt_print_filename(void *optctx, const char *opt, const char *arg) {
  struct ProbeContext* pctx = optctx; 
  pctx->print_input_filename = arg;
  return 0;
}

#if defined(BUILD_DLL)
void show_ffprobe_help_default(void* optctx, const char *opt, const char *arg) {
#else
void show_help_default(void* optctx, const char *opt, const char *arg) {
#endif
  struct ProbeContext* pctx = optctx; 
  if (!pctx) {
    return;
  }
  const OptionDef *options = pctx->options;
...
static int parse_read_intervals(const char *intervals_spec, struct ProbeContext* pctx) {
  int ret, n, i;
  ...
  pctx->read_intervals = av_malloc_array(n, sizeof(*pctx->read_intervals));
  if (!pctx->read_intervals) {
    ret = AVERROR(ENOMEM);
    goto end;
  }
  pctx->read_intervals_nb = n;
  ...
  for (i = 0; p; i++) {
    ...
    av_assert0(i < pctx->read_intervals_nb);
    next = strchr(p, ',');
    if (next)
      *next++ = 0;

    pctx->read_intervals[i].id = i;
    ret = parse_read_interval(p, &pctx->read_intervals[i]);
    if (ret < 0) {
    ...
    log_read_interval(&pctx->read_intervals[i], NULL, AV_LOG_VERBOSE);
    p = next;
  }
  av_assert0(i == pctx->read_intervals_nb);
...
static int opt_read_intervals(void *optctx, const char *opt, const char *arg) {
  return parse_read_intervals(arg, optctx);
}

static int opt_pretty(void *optctx, const char *opt, const char *arg) {
  struct ProbeContext* pctx = optctx; 
  pctx->show_value_unit = 1;
  pctx->use_value_prefix = 1;
  pctx->use_byte_value_binary_prefix = 1;
  pctx->use_value_sexagesimal_format = 1;
  return 0;
}
...
static void print_section(SectionID id, int level) {
  ...
  printf("%c%c%c%c", section->flags & SECTION_FLAG_IS_WRAPPER ? 'W' : '.',
...
#define SET_DO_SHOW(id, varname)                                               \
  do {                                                                         \
    if (check_section_show_entries(SECTION_ID_##id))                           \
      pctx->do_show_##varname = 1;                                                   \
  } while (0)


int init_options(struct ProbeContext *pctx) {
  pctx->show_private_data = 1;
  pctx->show_optional_fields = SHOW_OPTIONAL_FIELDS_AUTO;
  pctx->find_stream_info = 1;

  OptionDef real_options[]  = {
      CMDUTILS_COMMON_OPTIONS{
          "f", HAS_ARG, {.func_arg = opt_format}, "force format", "format"},
      {"unit", OPT_BOOL, {&pctx->show_value_unit}, "show unit of the displayed values"},
      {"prefix",
       OPT_BOOL,
       {&pctx->use_value_prefix},
       "use SI prefixes for the displayed values"},
      {"byte_binary_prefix",
       OPT_BOOL,
       {&pctx->use_byte_value_binary_prefix},
       "use binary prefixes for byte units"},
      {"sexagesimal",
       OPT_BOOL,
       {&pctx->use_value_sexagesimal_format},
      ...
      {"output_format",
       OPT_STRING | HAS_ARG,
       {&pctx->output_format},
       "set the output printing format (available formats are: default, compact, "
       "csv, flat, ini, json, xml)",
       "format"},
      {"print_format",
       OPT_STRING | HAS_ARG,
       {&pctx->output_format},
      ...
      {"show_data", OPT_BOOL, {&pctx->do_show_data}, "show packets data"},
      {"show_data_hash",
       OPT_STRING | HAS_ARG,
       {&pctx->show_data_hash},
      ...
      {"show_entries",
       ...
       "entry_list"},
#if HAVE_THREADS
      {"show_log", OPT_INT | HAS_ARG, {&pctx->do_show_log}, "show log"},
#endif
      ...
      {"count_frames",
       OPT_BOOL,
       {&pctx->do_count_frames},
       "count the number of frames per stream"},
      {"count_packets",
       OPT_BOOL,
       {&pctx->do_count_packets},
      ...
      {"show_private_data", OPT_BOOL, {&pctx->show_private_data}, "show private data"},
      {"private", OPT_BOOL, {&pctx->show_private_data}, "same as show_private_data"},
      {"bitexact", OPT_BOOL, {&pctx->do_bitexact}, "force bitexact output"},
      {"read_intervals",
       HAS_ARG,
  ...
  OptionDef* options = av_malloc(sizeof(real_options));
  memcpy(options, real_options, sizeof(real_options));
...
#ifdef BUILD_DLL
EXPORTED int ff_probe(int argc, char **argv, char **out, int *out_size)
#else
...
#endif
{
  ...
  int ret, input_ret, i;
  struct ProbeContext *pctx = NULL;

  pctx = av_calloc(1, sizeof(*pctx));

#ifndef BUILD_DLL
  init_dynload();
  pctx->do_show_error = 1;
#endif

  av_log_set_flags(AV_LOG_SKIP_REPEATED);

  init_options(pctx);
  parse_loglevel(argc, argv, pctx->options);
#ifndef BUILD_DLL
  ...
  show_banner(argc, argv, pctx->options);
  ret = parse_options(pctx, argc, argv, pctx->options, opt_input_file);
  if (ret < 0) {
  ...
  if (pctx->do_bitexact && (pctx->do_show_program_version || pctx->do_show_library_versions)) {
    av_log(NULL, AV_LOG_ERROR,
  ...
  if (!pctx->output_format)
    pctx->output_format = av_strdup("json");
  if (!pctx->output_format) {
    ret = AVERROR(ENOMEM);
    goto end;
  }
  w_name = av_strtok(pctx->output_format, "=", &buf);
  if (!w_name) {
  ...
  if (pctx->show_data_hash) {
    if ((ret = av_hash_alloc(&pctx->hash, pctx-> show_data_hash)) < 0) {
      if (ret == AVERROR(EINVAL)) {
        const char *n;
        av_log(
            NULL, AV_LOG_ERROR,
            "Unknown hash algorithm '%s'\nKnown algorithms:", pctx->show_data_hash);
  ...
  if ((ret = writer_open(&wctx, w, w_args, sections, FF_ARRAY_ELEMS(sections),
                         pctx->output_filename, pctx)) >= 0) {
    if (w == &xml_writer)
      wctx->string_validation_utf8_flags |=
    ...
    if (pctx->do_show_program_version)
      ffprobe_show_program_version(wctx);
    if (pctx->do_show_library_versions)
      ffprobe_show_library_versions(wctx);
    if (pctx->do_show_pixel_formats)
      ffprobe_show_pixel_formats(wctx);

    if (!pctx->input_filename &&
        ((pctx->do_show_format || pctx->do_show_programs || pctx->do_show_streams ||
          pctx->do_show_chapters || pctx->do_show_packets || pctx->do_show_error) ||
         (!pctx->do_show_program_version && !pctx->do_show_library_versions &&
          !pctx->do_show_pixel_formats))) {
      show_usage();
      ...
      av_log(NULL, AV_LOG_ERROR,
             ...
             program_name);
      ret = AVERROR(EINVAL);
    } else if (pctx->input_filename) {
      ret = probe_file(wctx, pctx->input_filename, pctx->print_input_filename);
      if (ret < 0 && pctx->do_show_error)
        show_error(wctx, ret);
    }
  ...
  clear_log(0);
  av_freep(&pctx->output_format);
  av_freep(&pctx->read_intervals);
  av_hash_freep(&pctx->hash);
